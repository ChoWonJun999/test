$(function() {
	...
	$("#btnInsertCalendar").on("click", function() {
		...
		// 날짜를 UTC 시간대로 변환
		var utcStartDate = new Date(Date.UTC(startDate.getFullYear(), startDate.getMonth(), startDate.getDate(), startDate.getHours(), startDate.getMinutes(), startDate.getSeconds()));
		...
		$.ajax({
			...
			type: "POST",
			data: $("#calendarForm").serialize() + "&start=" + utcStartDate.toISOString() + "&end=" + utcEndDate.toISOString(),
	...
	$("#btnDeleteCalendar").on("click", function() {
		Swal({
			title: '정말 삭제하시겠습니까?',
			html: "<span class='text-danger'>\"" + "#"+ $("#roomList").val() + " / "
						+ $("#bApcoName").val() +"\"</span> 일정이 삭제됩니다.",
			type: 'warning',
...
var selectDate = function(info) {
	...
	// room 초기화
	$("#roomList").empty();
	$.getJSON(cp + "/getRoomList").done(function(datas) { // 개인 일정
		var res = "<option value='' selected disabled hidden>==선택하세요==</option>";
		$.each(datas.roomList, function(idx, data) {
				res += "<option value='" + data.room_id + "'>" + data.roomNm + "</option>";
		}); 
		res += "</optgroup>";
		$("#roomList").append(res);
	});
	...
	$.getJSON(cp + "/getEmpCategoryList").done(function(datas) { // 개인 일정
		...
		$.each(datas.categoryList, function(idx, data) {
			if(data.category_nm == "현금"){
				res += "<option value='" + data.category_nm + "' selected>" + data.category_nm + "</option>";
			}
		}); 
		res += "</optgroup>";
	...
	$.getJSON(cp + "/getRoomList").done(function(datas) { // 개인 일정
		...
		$.each(datas.roomList, function(idx, data) {
			if(data.room_id === info.event.extendedProps.bApcoRoom) 
				...
				res += "<option ";
			res += "value='" + data.room_id + "'>" + data.roomNm + "</option>";
		}); 
		res += "</optgroup>";
		$("#roomList").append(res);
	});
	...
	$.getJSON(cp + "/getEmpCategoryList").done(function(datas) { // 개인 일정
		...
		$.each(datas.categoryList, function(idx, data) {
			if(data.category_id === info.event.extendedProps.category_id) 
				...
				res += "<option ";
			res += "value='" + data.category_nm + "'>" + data.category_nm + "</option>";
		}); 
		res += "</optgroup>";
		$("#categoryList").append(res);
	});
